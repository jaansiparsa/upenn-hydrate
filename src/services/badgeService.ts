...
export const awardBadge = async (userId: string, badgeId: string, progressData?: any): Promise<void> => {
...
export const checkAndAwardBadges = async (userId: string, actionType: string, actionData: any): Promise<string[]> => {
  ...
  try {
    ...
    const progressMap = new Map(userProgress.map(p => [p.progression_group, p]));
    ...
    for (const badge of allBadges) {
      ...
      switch (badge.requirement_type) {
        case 'count':
          shouldAward = await checkCountRequirement(userId, badge, actionType, actionData);
          break;
        case 'streak':
          shouldAward = await checkStreakRequirement(userId, badge, actionType, actionData);
          break;
        case 'special':
          shouldAward = await checkSpecialRequirement(userId, badge, actionType, actionData);
          break;
        case 'social':
          shouldAward = await checkSocialRequirement(userId, badge, actionType, actionData);
          break;
      }
...
const checkCountRequirement = async (userId: string, badge: Badge, actionType: string, actionData: any): Promise<boolean> => {
...
const checkStreakRequirement = async (userId: string, badge: Badge, actionType: string, actionData: any): Promise<boolean> => {
  if (actionType === 'review_created') {
...
const checkSpecialRequirement = async (userId: string, badge: Badge, actionType: string, actionData: any): Promise<boolean> => {
  switch (badge.name) {
    case 'First Sip':
    ...
    case 'Balanced Judge':
      if (actionType === 'review_created') {
        ...
        const hasAllCategories = data?.some(review => 
          review.coldness > 0 && 
          review.experience > 0 && 
          review.pressure > 0 && 
          review.yum_factor > 0
    ...
    case 'Ripple Effect':
      if (actionType === 'review_created') {
        ...
        const totalUpvotes = data?.reduce((sum, review) => 
          sum + (review.upvotes?.length || 0), 0) || 0;
...
const checkSocialRequirement = async (userId: string, badge: Badge, actionType: string, actionData: any): Promise<boolean> => {
  if (actionType === 'user_followed') {
    const { data } = await supabase
      .from('users')
      .select('following')
