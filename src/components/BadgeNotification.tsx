...
export const BadgeNotification: React.FC<BadgeNotificationProps> = ({
  ...
  return (
    <div className={`fixed top-4 right-4 z-50 transform transition-all duration-300 ${
      ...
      <div className={`bg-white rounded-lg shadow-lg border-2 border-gray-200 overflow-hidden max-w-sm w-full ${tierGlow[badgeTier]} shadow-lg`}>
        {/* Header with gradient */}
        <div className={`bg-gradient-to-r ${tierColors[badgeTier]} p-4 text-white`}>
        ...
        <div className="p-4">
          <div className="flex items-center space-x-4">
            <div className={`p-3 rounded-full ${tierColors[badgeTier]} bg-gradient-to-r text-white text-2xl`}>
              {badgeIcon}
            </div>
            <div className="flex-1">
              ...
              <div className="flex items-center mt-2">
                <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                  badgeTier === 'bronze' ? 'bg-amber-100 text-amber-800' :
                  badgeTier === 'silver' ? 'bg-gray-100 text-gray-800' :
                  badgeTier === 'gold' ? 'bg-yellow-100 text-yellow-800' :
                  badgeTier === 'platinum' ? 'bg-blue-100 text-blue-800' :
...
export const useBadgeNotifications = () => {
  ...
  const showBadgeEarned = (badgeName: string, badgeIcon: string, badgeDescription: string, badgeTier: 'bronze' | 'silver' | 'gold' | 'platinum' | 'special') => {
    addNotification({
      badgeName,
      badgeIcon,
      badgeDescription,
